...
float rand(float3 co){
    return frac(sin(dot(co.xyz, float3(12.9898, 72.833, 53.385))) * 43789.5432);
}
...
void LitPassGeom(triangle Varyings input[3], inout TriangleStream<Varyings> outStream){
    ...
    float randomHeight = rand(input[0].positionWSAndFogFactor.xyz * 62.1337f);
...
half4 LitPassFragment(Varyings input, bool vf : SV_IsFrontFace) : SV_Target
{
    ...
    color = lerp(color, _RootColor, 1 - input.uv.y);
    color = lerp(_RootColor, color, clamp(mainLight.shadowAttenuation + _ShadowPower, 0, 1));
